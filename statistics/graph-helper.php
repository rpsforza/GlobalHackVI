...
if (isset($_POST)) {
	...
	foreach ($options as $option) {
		...
		for ($i = $min_date; $i >= $max_date; $i -= $interval) {
			...
			for ($j = $i; $j < $i + $interval; $j++) {
				...
				if ($option === "intake" || $option === "output" || $option === "vacancy") {
					...
					$query = "SELECT * FROM provided_services WHERE date='$date' AND host_or_coc='$coc_or_host' AND provider_id=$provider_id" . $clause . $service_clause;
		...
		switch ($option) {
			case "intake":
				$rgb = "rgba(100, 0, 0, 0.4)";
				break;
			case "output":
				$rgb = "rgba(175, 15, 15, 0.4)";
				break;
			case "vacancy":
				$rgb = "rgba(250, 30, 30, 0.4)";
				break;
			case "all":
				$rgb = "rgba(0, 100, 0, 0.4)";
				break;
			case "completed":
				$rgb = "rgba(15, 175, 15, 0.4)";
				break;
			case "initiated":
				$rgb = "rgba(30, 250, 30, 0.4)";
				break;
			case "reservations":
				$rgb = "rgba(0, 0, 100, 0.4)";
				break;
			case "missed_reservations":
